...
export default function CartDrawer({ open, onClose }: CartDrawerProps) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      // Save each cart item as a separate order row in DB
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className="fixed top-0 right-0 h-full z-50 flex flex-col w-full sm:w-[360px]"
        style={{
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 flex-shrink-0"
          ...
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-black tracking-[0.2em] uppercase"
              style={{ fontFamily: "Georgia, serif", color: "#1a0a00" }}>Cart</h2>
            {count > 0 && (
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 ? (
            ...
            <div className="flex flex-col gap-5">
              {items.map((item) => (
                <div key={item.id} className="flex gap-3 pb-5"
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/products/${item.id}`} onClick={onClose}>
                      <p className="text-xs font-bold leading-snug hover:text-[#8B6914] transition-colors truncate"
                        style={{ color: "#1a0a00", fontFamily: "Georgia, serif" }}>{item.name}</p>
                    </Link>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => removeFromCart(item.id)}
                          className="transition-colors hover:text-red-400" style={{ color: "#a89070" }}>
                          <X size={13} />
                        </button>
                      </div>
                    </div>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-black tracking-[0.25em] uppercase" style={{ color: "#8B6914" }}>Order Note</label>
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="px-6 py-5 flex flex-col gap-3 flex-shrink-0"
